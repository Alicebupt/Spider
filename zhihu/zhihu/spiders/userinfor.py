...
class UserinforSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for eve_user in temp_data:
            ...
            item["utype"] = eve_user["type"]

            with open("zhihu/spiders/userinfor.txt", 'r')  as f:
                user_list = f.read()
            if eve_user['url_token'] not in user_list:
                with open("zhihu/spiders/userinfor.txt", 'w') as f:
                    f.write(eve_user['url_token']+"----")
                yield item

                new_url = 'https://www.zhihu.com/api/v4/members/" + item["url_token"] + "/followers?include=data%5B*%5D.answer_count%2Carticles_count%2Cgender%2Cfollower_count%2Cis_followed%2Cis_following%2Cbadge%5B%3F(type%3Dbest_answerer)%5D.topics&offset=60&limit=20'
